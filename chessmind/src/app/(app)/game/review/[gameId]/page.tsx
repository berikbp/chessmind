...
import { GameReviewClient } from "@/components/board/GameReviewClient";
...
export default async function GameReviewPage({ params }: Props) {
  ...
  const playerColor: "w" | "b" = game.white_id === user.id ? "w" : "b";

  return (
    <main className="mx-auto max-w-6xl px-0 py-2 sm:px-4 sm:py-6">
      <GameReviewClient game={{ ...game, pgn: game.pgn }} playerColor={playerColor} />
    </main>
  );
}
